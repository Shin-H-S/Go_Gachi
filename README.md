...
디지털 마케팅의 중요성이 커지면서 소상공인들의 광고 이미지 제작 수요도 꾸준히 늘고 있습니다. 그러나 소상공인 다수는 전문 디자인 도구 활용에 어려움을 겪고, 외주 제작에 시간과 비용 부담을 안고 있습니다. 특히 **인스타그램·배달앱·지역 커뮤니티(당근마켓)** 등 채널마다 요구하는 이미지 규격과 시각적 표현 방식이 달라, 같은 메뉴 사진을 매번 반복 제작해야 하는 비효율이 발생합니다.

### 2. 서비스 정의

**Go Gachi**는 소상공인이 보유한 원본 메뉴 사진 한 장을 업로드하면, 광고 채널과 상세 규격에 맞춰 최적화된 광고 이미지를 자동으로 생성하는 **AI 기반 광고 제작 솔루션**입니다.

디지털 도구에 익숙하지 않은 사용자도 쉽게 사용할 수 있도록 **업종 → 채널 → 유형 → 요청사항·문구 옵션** 순서의 직관적인 워크플로우를 제공합니다.
...
- **매체별 규격 대응**: 인스타그램(정사각/세로형/스토리), 배달의민족(단색/공간 배경), 당근마켓(메뉴/할인 이벤트) 총 **3개 채널 7가지 레이아웃 규격** 자동 보정.
- **상품성 보존**: 사용자가 업로드한 원본 상품의 형태, 색상, 질감을 왜곡 없이 유지.
- **비동기 생성 구조 구축**: 이미지 생성 모델의 긴 추론 시간(Latency) 동안 UI가 멈추지 않도록 **비동기 Job 시스템** 도입.
- **3대 문구 모드 지원**: 입력 문구 그대로 사용(Preserve), 자연스럽게 다듬기(Polish), 홍보 문구로 변환(Rewrite) 기능 제공 및 미입력 시 자동 생성.
...
- **도메인 환각(Hallucination) 리스크 관리**: 이미지 모델이 텍스트를 렌더링할 때 철자 오류가 나거나, 플랫폼 특성을 과해석하여 음료명을 잘못 기재(예: 당근 주스 오작명)하는 이슈가 관찰됨. 향후 시스템 프롬프트 제어력(Negative Prompting)을 고도화할 계획.
- **이미지 품질 의존성 극복**: 저품질 원본에 대한 품질 저하 현상을 막기 위해, 프론트엔드 내 '업로드 이미지 촬영 가이드' 제공을 강화.
- **UX 및 아키텍처 고도화**: 동기식 `/api/generate` 흐름을 완전히 폐기(Deprecated)하고 비동기 Job 단일 구조로 고도화. 문구 생성과 이미지 합성을 UI에서 분리하여 체감 대기 시간을 개선할 예정.
