...
def _copy_system_prompt(copy_mode: CopyMode) -> str:
    ...
    return (
        ...
        "Do not use output format names, platform placement names, or layout type names "
        "as advertising copy content. For Instagram copy generation, never use the word "
        "'스토리' in headline, subcopy, or cta unless the user explicitly provided it "
        "as part of the original marketing message or product copy. "
        "Do not describe the image format, canvas type, feed type, story placement, "
        "or rendering destination inside generated advertising text. "
